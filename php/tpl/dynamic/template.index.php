...
if (!function_exists('TemplateHeader'))
{
	...
	{
		global $themeurl, $user, $scripturl, $content, $spiel, $uni_presets;
...
if (!function_exists('TemplateFooter'))
{
	...
	<div id="footer">';
		...
		Browser: ', $_SERVER['HTTP_USER_AGENT'];
			...
			echo '</div>';
			if($content['debug_mode'] >= 2) {
				echo '<br /><a href="javascript:toggleVisibility(getElById(\'footer_req\'));">Req</a><span style="display:none;" id="footer_req"><br />';
				ob_start();
				var_dump($_REQUEST);
				$ob = ob_get_clean();
				...
				echo '<br /><a href="javascript:toggleVisibility(getElById(\'footer_content\'));">Content</a><br /><span style="display:none;" id="footer_content">';
				ob_start();
				var_dump($content);
				$ob = ob_get_clean();
				echo str_replace(array("<pre class='xdebug-var-dump' dir='ltr'>", "</pre>"), array('<span style="font-family: courier, monospace;">', "</span>"), $ob);
